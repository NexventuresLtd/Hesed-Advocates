...
export const Navbar = () => {
  ...
  useEffect(() => {
    // ⭐ 1. Read user preference, otherwise default to dark
    const storedMode = sessionStorage.getItem('darkMode');
    const darkPref = storedMode === null ? true : storedMode === 'true';

    // ⭐ 2. Persist the default when nothing was stored
    if (storedMode === null) sessionStorage.setItem('darkMode', 'true');
  ...
  return (
    ...
    >
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-r from-lime-500/5 to-green-500/5 pointer-events-none" />

      <div className="relative z-10 max-w-full md:max-w-11/12 mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-1">
            ...
            <a
              href="/contact"
              className="ml-4 px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 transform hover:scale-105 shadow-lg bg-gradient-to-r from-white to-lime-300 text-black"
            >
              Book a Consultation
            </a>
          </div>
